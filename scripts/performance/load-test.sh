...
THROUGHPUT=$(echo "scale=1; $COMPLETED_TICKETS * 60 / $DURATION" | bc -l 2>/dev/null || echo "0")
...
# Throughput check
if (( $(echo "$THROUGHPUT >= 50" | bc -l 2>/dev/null || echo "0") )); then
...
# Completion check
COMPLETION_RATE=$(echo "scale=1; $COMPLETED_TICKETS * 100 / $TOTAL_TICKETS" | bc -l 2>/dev/null || echo "0")
if (( $(echo "$COMPLETION_RATE >= 99" | bc -l 2>/dev/null || echo "0") )); then
...
if (( $(echo "$THROUGHPUT >= 50" | bc -l 2>/dev/null || echo "0") )) && [ $CONSISTENCY_RESULT -eq 0 ]; then
