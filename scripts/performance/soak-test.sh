...
TICKET_COUNTER=0
INTERVAL=$(echo "scale=2; 60 / $TICKETS_PER_MIN" | bc -l 2>/dev/null || echo "2")
...
MEM_DIFF=$(echo "$FINAL_MEM - $INITIAL_MEM" | bc -l 2>/dev/null || echo "0")
MEM_INCREASE_PCT=$(echo "scale=1; $MEM_DIFF * 100 / $INITIAL_MEM" | bc -l 2>/dev/null || echo "0")

if (( $(echo "$MEM_INCREASE_PCT < 20" | bc -l 2>/dev/null || echo "1") )); then
